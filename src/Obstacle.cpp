...
void Obstacle::setAmpliSources(const std::list<Wave*> &waves, int index, int t) {
...
void Obstacle::setTransfeMatrix(int index, int t) {
...
void Obstacle::setAmpliSources(const std::list<Wave*> &waves, int index) {
...
void Obstacle::setTransfeMatrix(int index) {
  INFO("Set Transfer matrix:  "<<index<<"  (wavelength "<<wave_lenghts[index]<<", "<<sources_l[index].size()<<" sources)");
  if (boundaries_l[index].size() != 0) {
    std::vector<EquivalentSource*> &sources = sources_l[index];
    std::vector<InputPoint*> &boundaries = boundaries_l[index];
...
void Obstacle::reset(const std::vector<std::list<Wave*> > &waves_l, const std::vector<FLOAT> &wl) {
 ...
 int nm = 0;
 int j = 0;
 int nb_threads = NTHREADS_;
 while (nm < nb_wl) {
   nm = std::min((j+1)*nb_threads, nb_wl);
#pragma omp parallel for
   for (int i = j*nb_threads; i < nm; ++i) {
     setBoundaries(i);
...
void Obstacle::getBoundariesPos(std::list<VEC2> &boundaryPos) const {
  for (int w = 0; w < nb_wl; ++w) {
    for (uint i = 0; i < boundaries_l[w].size(); ++i) {
      boundaryPos.push_back(boundaries_l[w][i]->getPos());
    }
    for (uint i = 0; i < sources_l[w].size(); ++i) {
      boundaryPos.push_back(sources_l[w][i]->getPos());
    }
...
void Obstacle::getSourcesPos(std::list<VEC2> &sourcesPos) const {
  for (int w = 0; w < nb_wl; ++w) {
    for (uint i = 0; i < sources_l[w].size(); ++i) {
      sourcesPos.push_back(sources_l[w][i]->getPos());
    }
    for (uint i = 0; i < sources_l[w].size(); ++i) {
      sourcesPos.push_back(sources_l[w][i]->getPos());
    }
...
COMPLEX Obstacle::heightc_wl(FLOAT x, FLOAT y, int w) const{
  COMPLEX a = 0;
...
COMPLEX Obstacle::heightc_wl(FLOAT x, FLOAT y, int w, FLOAT time) const{
  COMPLEX a = 0;
...
void Obstacle::getSources(std::list<Wave*> &waves, int w) {
  if (!movable_) {
    for (uint i = 0; i < sources_l[w].size(); ++i) {
...
void Obstacle::getSourcesM(std::list<MovingEquivalentSource*> &waves, int w) {
  if (movable_) {
    for (uint i = 0; i < sources_l[w].size(); ++i) {
...
std::vector<InputPoint*> &Obstacle::getBoundaries(int w) {
  return boundaries_l[w];
}

std::vector<VEC2C> &Obstacle::getNormales(int w) {
  return normals_l[w];
}

std::vector<EquivalentSource*> &Obstacle::getSources(int w) {
  return sources_l[w];
}
		    

int Obstacle::getBoundariesSize(int w) const {
  return boundaries_l[w].size();
}

int Obstacle::getSourcesSize(int w) const {

...
void TextureObstacle::setBoundaries(int w) {
  ...
  INFO("nb bound "<< boundaries.size());
}


void TextureObstacle::setEquivalentSources(int w) {
  FLOAT wl = wave_lenghts[w];
#ifdef INTERACTIVE_
  if (wl >= 20*cell_size || cell_size*grid.getNbRows() < wl) {
    return;
  ...
  int k_max = wl*offset_/cell_size;
  if (k_max < 1) {
    k_max = 1;
  }
  if (k_max > 25) {
...
TextureObstacle::TextureObstacle(std::string file, int nr, int nc, FLOAT cs): Obstacle() {
  n_rows = nr;
  n_cols = nc;
  cell_size = cs;

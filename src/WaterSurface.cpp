...
void WaterSurface::reset() {
  ...
  sphere_bp.setSize(0.1);
  sphere_bp.setColor(0.0f, 1.0f, 1.0f);
  sphere_pp.setSize(0.05);
  sphere_pp.setColor(0.5f, 0.1f, 1.0f);
  sphere_pp2.setSize(0.1);
  sphere_pp2.setColor(1.0f, 0.1f, 1.0f);
...
//     const std::vector<EquivalentSource*> pattern_sources = wave_pattern.getSources();
//     for (auto it: pattern_sources) {
// #pragma omp parallel for
//       for (int i = 0; i < n_rows_ - 1; i++) {
// 	FLOAT x = cell_size_*i;
// 	for (int j = 0; j < n_cols_ - 1; j++) {
// 	  FLOAT y = cell_size_*j;
// 	  ampli_scattered_re[w](i, j) += real(it->heightc(x, y, 0));
// 	  ampli_scattered_im[w](i, j) += imag(it->heightc(x, y, 0));
// 	}
//       }
//     }
...
// 	for (int j = 0; j < n_cols_ - 1; j++) {
// 	  FLOAT y = cell_size_*j;
// 	  ampli_scattered_re[w](i, j) += real(it->heightc(x, y, t));
// 	  ampli_scattered_im[w](i, j) += imag(it->heightc(x, y, t));
// 	}
//       }
//     }
...
FLOAT WaterSurface::height_obs(int i, int j) const {
  if (b(i, j) == 0) {
    return -1;
  }
  return b(i, j);
...
void WaterSurface::addLinearWave(FLOAT x, FLOAT y, FLOAT wl, COMPLEX ampli) {
  ...
  for (int index = 0; index < nb_wl && wave_lenghts[index]<=wl; ++index) {
...
void WaterSurface::addDropW(VEC2 pos, FLOAT size, int t) {
  std::cout<<"add drop "<<pos<<"  "<<size<<" "<<t<<std::endl;  
#ifdef INTERACTIVE_
...
void WaterSurface::update() {
  ...
  if (export_) {
    if (time%export_step == 0) {
      ...
      INFO(export_file<<time/export_step<<".grid");
      std::string str(ss.str());
...
void WaterSurface::updateHeight() {
  ...
  for (int f = 0; f < nb_wl; ++f) {
    ...
    // std::cout<<"show in field "<< show_in_field<<std::endl;
    if (show_in_field) {
#pragma omp parallel for
      for (int i = 0; i < n_rows_ - 1; i++) {
	for (int j = 0; j < n_cols_ - 1; j++) {
...
void WaterSurface::draw() {
  ...
  if (draw_sources) {
  std::list<VEC2> boundariesPos;
  getObstacleBoundary(boundariesPos);
  std::list<VEC2> sourcesPos;
  getObstacleSources(sourcesPos);
  //  wave_pattern.getSourcesPos(sourcesPos);
  std::list<VEC2> ppPos;
  // wave_pattern.getPatternPointsPos(ppPos);
   ...
   for (auto&it : boundariesPos) {
     VEC2 c = it;//world2viewer(it);
     glPushMatrix();
     glTranslatef(c[0], c[1], 1.0f);
     sphere_bp.draw();
     glPopMatrix();
...
void WaterSurface::getObstacleSources(std::list<VEC2> &sourcesPos) const {
  std::list<Obstacle*>::const_iterator ito;
  for (ito = obstacles.begin(); ito != obstacles.end(); ++ito) {
    (*ito)->getSourcesPos(sourcesPos);
  }
}
...
void WaterSurface::exportSurfaceFreq(std::string file) const {
  VERBOSE(1, "Exporting frequency grids: "<<file<<std::endl);
...
void WaterSurface::importSurfaceFreq(std::string file) {
  VERBOSE(1, "Importing frequency grids: "<<file<<std::endl);
...
void WaterSurface::exportSurfaceObj(std::string file) const {
  VERBOSE(1, "Exporting surface OBJ: "<<file);
  std::ofstream os(file.c_str());
  ERROR(os.good(), "Cannot open file "<<file, "");
#ifdef PROJECTED_GRID
...
void  WaterSurface::exportObstacleGrid(std::string file) const {
  VERBOSE(1, "Exporting obstacle OBJ: "<<file);
  std::ofstream os(file.c_str());
  ERROR(os.good(), "Cannot open file "<<file, "");
  b.exportObj(os);
  os.close();
...
void WaterSurface::importConfig(std::string file) {
  ...
  while (getline(is, line)) {
	...
	getline(is, line);
     ...
     } else  if (line.substr(0,9) == "<pattern>") {
       getline(is, line);
       std::string name;
       int nc = 200;
       int nr = 200;
	  ...
	  s >> m;
       ...
       INFO("pattern file " << file);
       // wave_pattern = WavePattern(wave_lenghts[0]);
       // wave_pattern.setPos(x, y);
       // wave_pattern.setAmpli(ampli);
       // wave_pattern.setInitVal(iv);
       // wave_pattern.setMethod(m);
       // wave_pattern.setSize(nr, nc, cs);
       // wave_pattern.createPatternPoints(file);
       // wave_pattern.createSources(radius);
       // wave_pattern.setTransferMatrix();
       //   wave_pattern.solve(waves[0]);
    } else {
...
void WaterSurface::setExportStep(int es) {
  export_step = es;
}
...
void WaterSurface::setCuda() {
  ...
  for (int w = 0; w < nb_wl; ++w) {
    ...
    int nb_sources = waves_wl.size();
    int nb_sources_input = waves_wl_input.size();
...
void WaterSurface::setProjGrid(int i, int j, FLOAT x, FLOAT y) {
  proj_grid.setPosOnThePlane(i, j, x, y);
}

VEC3 WaterSurface::getPosProjGrid(int i, int j) const {
...
VEC3 WaterSurface::getPosProjGrid(int i) const {
  VEC3 p = proj_grid(i);
